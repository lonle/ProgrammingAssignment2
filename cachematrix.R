...
# The <<- operator which can be used to assign a value to an object in an environment that is different from 
# the current environment. Below are two functions that are used to create a special object that stores a 
...
# The second function, cacheSolve, checks to see if the inverse is cached.  If it is cached then it will retrieve
# the inverse from cache.  Otherwise, it will calculate the inverse
